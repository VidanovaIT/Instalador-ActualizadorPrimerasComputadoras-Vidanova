...
# Descarga y ejecuta SDI Lite para instalar drivers automáticamente
...
# Esta función instala Winget si no está disponible.
#
function InicializarWinget {
    Write-Host "`n--- Verificación inicial de Winget ---`n"
    ...
    if (!(Get-Command winget -ErrorAction SilentlyContinue)) {
        ...
        try {
            ...
            throw "Winget no disponible tras intento de instalación."
        }

        # Verifica nuevamente si está disponible
        if (!(Get-Command winget -ErrorAction SilentlyContinue)) {
            throw "Winget no disponible tras intento de instalación."
        }
    }
    ...
    Write-Host "Si es la primera vez, puede pedir aceptar términos. Presiona Y y ENTER si corresponde.`n"

    winget list --accept-source-agreements --upgrade-available

    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Winget no pudo listar los paquetes. Verifica tu conexión a Internet o la instalación de Winget."
        Read-Host "Presiona ENTER para continuar..."
        exit 1
    }
}

# Esta función abre una URL en el navegador predeterminado o en uno específico si se encuentra.
...
function AbrirEnNavegador {
    ...
    if ($browser) {
        ...
        Write-Log "No se detectó navegador moderno, abriendo con el predeterminado (Edge/IE)." "WARNING"
        Start-Process $url
    }
}

# Esta función descarga un archivo desde una URL y lo instala si es un ejecutable o MSI.
# Si la descarga falla o el archivo no es válido, abre una página de instalación manual.
...
# Esta función instala y actualiza una lista de programas comunes usando Winget.
...
function InstalarYActualizarProgramas {
    ...
    foreach ($programa in $programas) {
        ...
        if (-not $estaInstalado) {
            ...
            # Si está instalado, verificar si hay actualización
...
# Esta función descarga fondos de pantalla y un protector de pantalla, y los configura.
...
# Esta función configura Lively Wallpaper como protector de pantalla y establece un fondo de escritorio.
